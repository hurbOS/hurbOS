...
void *PREFIX(malloc)(uint64_t req_size)
{
	int startedBet = 0;
...
void* PREFIX(calloc)(uint64_t nobj, uint64_t size)
{
       int real_size;
       void *p;
...
void*   PREFIX(realloc)(void *p, uint64_t size)
{
	void *ptr;
	struct liballoc_minor *min;
...
void *liballoc_alloc(uint64_t size) {
	return pmm_malloc(size * PAGE_SIZE);
}

uint64_t liballoc_free(void* ptr, uint64_t size) {
	pmm_free(ptr, size * PAGE_SIZE);
	return 0;
}
